...
const LifeStyleMatrixSummary = ({ lifestyleMatrixReport }) => {
  const answers = lifestyleMatrixReport?.matrix_answers || {};

  if (!Object.keys(answers).length) {
    return (
      <div className="bg-white rounded-[32px] shadow-xl p-8 mb-8">
        <h2 className="text-2xl font-bold mb-6">Lifestyle Matrix</h2>

        <div className="text-slate-500">No lifestyle matrix data available</div>
      </div>
    );
  }
    ...
    <div className="bg-white rounded-[32px] shadow-xl p-8 mb-8">
      <div className="flex items-center gap-3 mb-8">
        <div className="w-12 h-12 rounded-2xl bg-[#46C18D]/10 flex items-center justify-center">
          <ChevronRight size={22} className="text-[#46C18D]" />
        </div>
