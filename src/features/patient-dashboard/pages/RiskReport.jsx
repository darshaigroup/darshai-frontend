...
export default function RiskReport() {
  ...
  const metrics = useMemo(
    () => [
      { title: "Composite Score", value: report?.composite_score ?? assessment?.composite_score ?? "--", color: "emerald" },
      { title: "Risk Band", value: report?.risk_band ?? assessment?.risk_band ?? "--", color: "red" },
      { title: "Domains", value: ai?.blocks?.length ?? 0, color: "blue" },
      { title: "Completion", value: `${ai?.total_completion_pct ?? 0}%`, color: "amber" },
    ],
    [report, assessment, ai]
  );
        ...
        <div className="flex flex-col gap-6 border-b border-slate-200 p-8 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <button
              ...
              className="mb-5 inline-flex items-center gap-2 text-sm font-medium text-slate-500 hover:text-emerald-600"
            >
          ...
          <div className="flex gap-3">
            <button
              ...
              className="flex items-center gap-2 rounded-xl border border-slate-300 px-5 py-3 text-sm font-semibold hover:bg-slate-100"
            >
              <Printer className="h-4 w-4" />
              Print
            </button>

            <button className="flex items-center gap-2 rounded-xl bg-[#06152A] px-5 py-3 text-sm font-semibold text-white hover:bg-[#0A2342]">
