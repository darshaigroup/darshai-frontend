...
export default function ClinicalReport(){
  ...
  const metrics=useMemo(()=>[
    ...
    {title:"Primary Goal",value:clinical?.primaryGoal??"--",color:"blue"},
    {title:"Medical Conditions",value:clinical?.medicalConditions?.length??0,color:"amber"}
  ],[report,assessment,clinical]);

  const generatedDate=new Date(
    report?.updated_at??report?.created_at??assessment?.created_at??patient?.updated_at??Date.now()
  ...
  const downloadPDF=async()=>{
    ...
    try{
      ...
      const pageWidth=210,pageHeight=297,margin=12,headerHeight=20,footerHeight=10;
      const contentWidth=pageWidth-margin*2;
      const scale=contentWidth/canvas.width;
      const sliceHeight=Math.floor((pageHeight-margin-headerHeight-footerHeight)/scale);
      ...
      const safeName=(patient?.name||"Clinical")
        .replace(/[^a-z0-9]+/gi,"-")
        .replace(/^-|-$/g,"");

      pdf.save(`DarshAI-Clinical-Report-${safeName||"Patient"}.pdf`);
    }catch(error){
      console.error("CLINICAL PDF DOWNLOAD ERROR:",error);
  ...
  return(
    <div className="mx-auto max-w-7xl space-y-8">
      ...
      <style>{`
        ...
        @media print{
          @page{size:A4;margin:22mm 12mm 16mm}
