...
export default function LifestyleReport(){
  ...
  const downloadPDF=async()=>{
    ...
    try{
      ...
      const safeName=(patient?.name||"Lifestyle")
        .replace(/[^a-z0-9]+/gi,"-")
        .replace(/^-|-$/g,"");

      pdf.save(`DarshAI-Lifestyle-Report-${safeName||"Patient"}.pdf`);
    }catch(error){
      console.error("LIFESTYLE PDF DOWNLOAD ERROR:",error);
