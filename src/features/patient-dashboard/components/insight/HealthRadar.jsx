    ...
    <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-xl">
      ...
      <div className="mt-3 space-y-2 text-sm">
        ...
        <div className="flex justify-between gap-6">
          ...
          <span className={`rounded-full border px-2 py-0.5 text-xs font-semibold ${riskColor(d.score)}`}>
            {risk(d.score)}
          </span>
        </div>
        <div className="flex justify-between gap-6">
          <span className="text-slate-500">Answered</span>
          <span className="font-semibold">{d.answered}/{d.total}</span>
        </div>
      </div>
    </div>
  );
...
export default function HealthRadar({ assessment = {} }) {
  ...
  const high = blocks.filter(b => pct(b.score) >= 80).length;
  const moderate = blocks.filter(b => {
    const s = pct(b.score);
    return s >= 60 && s < 80;
  }).length;
