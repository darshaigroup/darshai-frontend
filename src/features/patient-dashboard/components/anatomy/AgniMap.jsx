...
export default function AgniMap({ patient }) {
  ...
  const agni=report?.agni_result||{};
  const score=Number(
    agni?.score||
    agni?.agni_score||
    patient?.compositeScore||
    82
  );
  ...
  const recommendation=
    ...
    "Continue warm hydration, eat freshly prepared meals, avoid late dinners and maintain a consistent eating schedule to strengthen digestive resilience.";
  ...
  const metabolism=
    agni?.metabolism||
    (score>=80?"Optimal":score>=60?"Moderate":"Needs Attention");
  ...
  const stages=[
    {
      name:"Mandagni",
      active:agniType==="Mandagni"||score<45,
    },
    {
      name:"Samagni",
      active:agniType==="Samagni"||(score>=45&&score<=80),
    },
    {
      name:"Tikshnagni",
      active:agniType==="Tikshnagni"||score>80,
    },
  ];
        ...
        <div>
          ...
          <div className="grid grid-cols-3 gap-2 sm:gap-3">
            ...
            {stages.map(stage=>(
              ...
              >
                <p className="text-[9px] sm:text-[10px] uppercase tracking-[.15em] text-slate-500">
                  {stage.name}
                </p>
