...
export default function DoshaBodyMap({ patient }) {
  ...
  const prakriti = report?.prakriti_result || {};
  const correlation = report?.correlation_result || {};

  const getLevel = (name) => {
    if (report?.primary_dosha === name)
      return Number(report?.primary_level || 0);
    if (report?.secondary_dosha === name)
      return Number(report?.secondary_level || 0);
    return 0;
  };

  const ayur = patient?.finalAyurvedaResult || {};
  const doshas = useMemo(
    () => ({
      pitta: {
        ...
        icon: Flame,
        color: "#F59E0B",
        level: `${getLevel("Pitta")}%`,
        badge:
          report?.primary_dosha === "Pitta"
            ? "Dominant"
            : report?.secondary_dosha === "Pitta"
              ? "Secondary"
              : "Balanced",
        description:
          ayur?.pitta?.description ||
          correlation?.pitta ||
          prakriti?.description ||
          "Controls metabolism, digestion, hormonal regulation and body temperature.",
        recommendation:
          ayur?.pitta?.recommendation ||
      ...
      vata: {
        ...
        level: `${getLevel("Vata")}%`,
        badge:
          report?.primary_dosha === "Vata"
            ? "Dominant"
            : report?.secondary_dosha === "Vata"
              ? "Secondary"
              : "Balanced",
        description:
          ayur?.vata?.description ||
          correlation?.vata ||
          prakriti?.description ||
          "Responsible for movement, nervous system activity, respiration and circulation.",
        recommendation:
          ayur?.vata?.recommendation ||
          prakriti?.recommendation ||
      ...
      kapha: {
        ...
        icon: Droplets,
        color: "#10B981",
        level: `${getLevel("Kapha")}%`,
        badge:
          report?.primary_dosha === "Kapha"
            ? "Dominant"
            : report?.secondary_dosha === "Kapha"
              ? "Secondary"
              : "Balanced",
        description:
          ayur?.kapha?.description ||
          correlation?.kapha ||
          prakriti?.description ||
          "Provides stability, immunity, lubrication and tissue nourishment.",
        recommendation:
          ayur?.kapha?.recommendation ||
          prakriti?.recommendation ||
          "Increase physical activity and reduce heavy foods.",
      },
    }),
    [report, prakriti, correlation, ayur],
  );
  ...
  <div className="mb-6 flex items-center justify-between">
    ...
    <div>
      ...
      <p className="mt-1 text-sm text-slate-500">
        Current constitutional balance
      </p>
  ...
  {[
    {
      name: "Pitta",
      color: "#F59E0B",
      value: getLevel("Pitta"),
      active: report?.primary_dosha === "Pitta",
    },
    {
      name: "Vata",
      color: "#38BDF8",
      value: getLevel("Vata"),
      active: report?.primary_dosha === "Vata",
    },
    {
      name: "Kapha",
      color: "#10B981",
      value: getLevel("Kapha"),
      active: report?.primary_dosha === "Kapha",
    },
  ].map((dosha) => (
        ...
        />
          ...
          <div className="w-full rounded-3xl border border-slate-200 bg-white p-6">
            ...
            <p className="leading-8 text-slate-600">
              {current.description || patient?.clinicalSummary}
            </p>
          </div>
          ...
          <div className="w-full rounded-3xl border border-emerald-100 bg-gradient-to-br from-emerald-50 via-white to-sky-50 p-6">
            ...
            <p className="mt-4 leading-8 text-slate-700">
              {current.recommendation || patient?.clinicalSummary}
            </p>
          </div>
        </div>
      </div>
