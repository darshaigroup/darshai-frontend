          ...
          {Object.entries(nodes).map(([key,node])=>{
            ...
            return(
              <motion.button
                ...
                style={{left:`${positions[key][0]}%`,top:`${positions[key][1]}%`}}
              >
                <motion.span
                  animate={{scale:active?[1,1.9,1]:[1,1.45,1],opacity:active?[.45,.12,.45]:[.3,.08,.3]}}
                  transition={{repeat:Infinity,duration:active?2.4:2.8,ease:"easeInOut"}}
                ...
                <motion.span
                  ...
                  style={{background:node.color,boxShadow:`0 0 14px ${node.color}80`}}
                />
              </motion.button>
            );
          })}
      ...
      <div className="space-y-6">
        <div className="rounded-[30px] border border-slate-200 bg-white p-6 shadow-sm">
          ...
          {/* <div
            ...
            style={{background:`linear-gradient(135deg,${current.color}12,#ffffff)`,borderColor:`${current.color}30`}}
          >
            <div className="flex items-center gap-2">
              <Sparkles size={18} style={{color:current.color}}/>
              <span className="font-mono text-[10px] font-semibold uppercase tracking-[.22em] sm:text-[11px]" style={{color:current.color}}>
                Clinical Interpretation
              </span>
            </div>
            <p className="mt-4 font-sans text-sm leading-8 text-slate-700 sm:text-base">{current.insight}</p>
          </div> */}
        </div>
      </div>
    </div>
