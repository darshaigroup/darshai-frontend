...
export default function WellnessCard({ title, value, status, trend = "", color = "emerald", insight = "", className = "" }) {
  ...
  const riskTheme = {
    Low: { bg: "bg-emerald-500/10", border: "border-emerald-500/20", text: "text-emerald-500", glow: "bg-emerald-500/5", badge: "bg-emerald-50 text-emerald-700", dot: "bg-emerald-500" },
    Moderate: { bg: "bg-amber-500/10", border: "border-amber-500/20", text: "text-amber-500", glow: "bg-amber-500/5", badge: "bg-amber-50 text-amber-700", dot: "bg-amber-500" },
    High: { bg: "bg-rose-500/10", border: "border-rose-500/20", text: "text-rose-500", glow: "bg-rose-500/5", badge: "bg-rose-50 text-rose-700", dot: "bg-rose-500" }
  };

  const theme = title === "Risk Tier" ? riskTheme[status] || colors.emerald : colors[color] || colors.emerald;
    ...
    <motion.div whileHover={{ y: -6, scale: 1.01 }} transition={{ duration: .25 }} className={`group relative overflow-hidden rounded-[30px] border border-stone-200 bg-white p-6 shadow-[0_18px_45px_rgba(15,23,42,.08)] transition-all hover:shadow-[0_28px_65px_rgba(15,23,42,.12)] ${className}`}>
      ...
      <div className="relative mt-7">
        <h2 className="text-3xl font-bold tracking-tight text-slate-900">
          {title === "Risk Tier" ? status : value}
        </h2>
      </div>

      <div className={`relative mt-6 rounded-2xl border ${theme.border} ${theme.bg} p-3.5 transition-all group-hover:shadow-sm`}>
        <div className="flex items-center gap-2">
          <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-white shadow-sm ${theme.text}`}>
            <HeartPulse className="h-4 w-4" />
          </div>
          <div className="min-w-0">
            <p className={`text-[10px] font-semibold uppercase tracking-[.15em] ${theme.text}`}>
              {syncLabel}
            </p>
          </div>
        </div>
      ...
      <div className="relative mt-5 flex items-center gap-2 text-xs text-slate-400">
        ...
        <span>{insight || "Personalized health profile synchronized"}</span>
      </div>
    </motion.div>
  );
}